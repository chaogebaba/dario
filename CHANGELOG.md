...
- **Claude Code is recognised by the block only Claude Code sends, not by a list of its opening lines.** `isGenuineCCClient` decides the most consequential branch in the request path: a genuine client's bytes are forwarded verbatim, everything else is rebuilt through the template and has its orchestration tags scrubbed first. It required `system[0]` to carry the `x-anthropic-billing-header` block *and* `system[1]` to start with one of five enumerated openers. CC is not five request shapes. The v2.1.236 bundle ships more than twenty distinct system prompts, and of sixteen real ones checked against the predicate, eleven were classified foreign — the code reviewer, the security reviewer, the web-reading specialist, workflow subagents, background observers, the claude-code-guide agent, and the summariser behind every `/compact` and every auto-compact. That last one is the expensive case: the foreign path runs `sanitizeMessages` over the transcript, so the `<system-reminder>` and `<env>` blocks were stripped out of the very conversation being summarised, and the request was reassembled from the template instead of forwarded as CC sent it. Enumerating a moving target was the mistake; the billing block is already the discriminator and no other client emits one. The opener list is gone, and with it the documented custom-agent gap — a `~/.claude/agents` definition puts operator-authored text at `system[1]` with no marker to key on, which is precisely why it used to ride the template path. What the list was also doing, keeping a wrapper client that replays CC's preamble under its own identity off the passthrough path, moves to `detectTextToolClient`, which already knows those clients by name and does not have to be told about CC's next release to keep working.
