...
- **Voice dictation works behind an egress proxy. It used to refuse.** The relay dials with `node:https`, and `installOutboundProxyWrapper` only wraps `globalThis.fetch`, so with `--egress-proxy` set the relay had no way to honour it — and correctly declined with a 502 rather than sending the voice socket out by the default route while every other upstream call went through the proxy. The effect was that voice was dead for everyone running an egress proxy, which for a SOCKS proxy is everyone who needs one at all: Bun's fetch cannot speak SOCKS either, which is why `socks5-bridge.ts` exists. That bridge is a loopback HTTP proxy that accepts CONNECT, which is exactly the shape a raw TLS socket needs, so the relay now tunnels through it — CONNECT to the origin, then TLS over the tunnel with the origin's own hostname for SNI and certificate validation. The proxy sees a CONNECT and nothing else. An http/https egress speaks CONNECT natively and is used directly. The decline is still there, narrowed to the case it was always about: an egress proxy configured with no tunnel available.

  Confirmed against the live endpoint, which is the first time the relay has met one — every previous test drove a fake upstream. Through a SOCKS5 egress, `api.anthropic.com` answered `101 Switching Protocols` with a `Sec-WebSocket-Accept` that verifies against the key sent, a real `request-id`, Cloudflare's own headers, and held the socket open; the egress IP stayed the proxy's throughout. Two things that were only ever assumptions are now measured: Anthropic accepts a **pool** token on the voice socket rather than only the token CC minted for itself, and the handshake dario forwards — path, query order, and the four identity headers, all lifted from the CC bundle — satisfies the real endpoint.


- **Request-side fault injection, and the line it drew.** stream-fault-injection.mjs covered the upstream response going wrong; nothing covered the CLIENT going wrong — hanging up mid-upload, sending prose instead of JSON, sending JSON truncated mid-object, sending `[]` or `null` or a bare string, or exceeding MAX_BODY_BYTES. All of those land in the window that owns a queue slot, so a mishandled one costs capacity rather than just answering badly. dario turns out to split them cleanly, and the new suite pins the split rather than the behaviour that was easiest to assert: oversized bodies and client hangups are dario's own call (413 from dario, and never a pool token spent on a request that never finished arriving), while a merely unparseable body goes upstream verbatim and Anthropic's verdict is relayed untouched. The second half is the one worth having a test for — a validator in dario would be a second, private definition of what the Messages API accepts, and every point of disagreement would be a request that works against api.anthropic.com and fails against dario.


- **`BODY_READ_TIMEOUT_MS` is now clamped to the pool's token-expiry margin, and the margin has a name.** proxy.ts reads the bearer off the account it selects and never looks at the account again, so everything between selection and the outbound header has to fit inside the life select() guaranteed. The only unbounded thing in that gap is the request-body read — and the sole reason a slow uploader could not drain the margin was that `BODY_READ_TIMEOUT_MS` in proxy.ts and a bare `30_000` literal in pool.ts happened to be equal, in different files, with nothing tying them together and neither mentioning the other. The literal is now `TOKEN_EXPIRY_MARGIN_MS` (upstream's name for it) and the body timeout is `Math.min(30_000, TOKEN_EXPIRY_MARGIN_MS)`, so the two cannot drift apart. Nothing was broken: select() rejects `expiresAt <= now + margin` strictly, so an eligible token has more than 30s left and the body read is destroyed at exactly 30s — the read always loses that race. The hypothesis going in was the opposite, and test/token-expiry-midbody.mjs records the disproof alongside the guard so the next reader does not re-hunt it. Raising the body timeout past the margin is what would break it: dario sends a dead bearer, Anthropic 401s, and a seat with sound credentials drops into auth cool-down for someone else's slow upload.


- **Merged upstream v5.5.25 through v5.5.43.** Four of the twenty-three commits fix bugs this fork had already found and fixed independently, so the merge was a reconciliation rather than a fast-forward: `/health` asking select()'s own question (#1030), the superset rule on a live template load (#1035), utilisation freshness on the account surfaces (#1032), and never ending a request on an assistant turn (#1033). Where both sides had a fix, the wider one survives — dario's `ineligibleReason` covers `disabled` and per-family rate-limit cool-downs that upstream's `accountIneligibility` does not, and `poolVerdict` already gave the four "can this pool serve" surfaces one answer.

- **`/accounts` and `/admin/accounts` rename `measuredAt` to upstream's `lastObservedAt`, and gain `utilAgeMs`.** Both projects surfaced the same `rateLimit.updatedAt` under different names; upstream's is the one with a public contract, and it encodes "never observed" as `null` rather than `0`. The snake_case surface follows as `last_observed_at` / `util_age_ms`. The TUI reads the new field and falls back to `measuredAt` when it is talking to a proxy that predates the rename.

- **Kept dario's whole-bundle tool union over upstream's classified one, with the measurement that settles it.** Upstream's #1035 restores only tools listed in `PLATFORM_ONLY_TOOLS`, `INTERACTIVE_ONLY_TOOLS` or `CONFIG_SCOPED_TOOLS`. Against the CC 2.1.236 headless capture the test fixtures model, that preserves 10 of the 24 tools such a capture drops and loses the other 14 — Cron*, Web*, `TaskOutput`, `TaskStop`, `NotebookEdit`, `SendMessage`, `Monitor`, `DesignSync`, the worktree pair, `PushNotification`. `TaskOutput` and `TaskStop` are the tell: `CONFIG_SCOPED_TOOLS` registers the other four Task tools and omits those two because they "stayed put" in the capture upstream measured, which is no longer true. Losing a tool is the expensive direction by upstream's own reasoning — `CC_NATIVE_NAMES_UNION` derives from the loaded template, so a client declaring a dropped tool has its history renamed onto a fallback slot with junk arguments. A hand-maintained allowlist rots against the thing it tracks; the whole-bundle union does not.


## [5.6.0] - 2026-08-20

- **Voice dictation was never broken behind dario. It was bypassing dario, carrying your own OAuth token.** The premise everyone including this project had been working from was that Claude Code's `/voice` WebSocket hits `ANTHROPIC_BASE_URL` and dario, being a plain `node:http` server with no upgrade handling, simply dropped it. Reading the v2.1.237 bundle says otherwise: `connectVoiceStream` builds its URL from `getOauthConfig().BASE_API_URL`, which is the string `https://api.anthropic.com` compiled into the prod build and overridable only through `CLAUDE_CODE_CUSTOM_OAUTH_URL` against a three-host first-party allowlist dario is not on. `ANTHROPIC_BASE_URL` appears nowhere in that path. So voice has been working the whole time, for everyone, straight to Anthropic — and the socket it opens carries `Authorization: Bearer <the user's own OAuth access token>`, read from `~/.claude/.credentials.json` and refreshed immediately before the handshake. That is the one credential a dario user points everything else at a local proxy to keep off the wire under their own name, leaving the box on its own socket, on a path nothing in dario could see. Fixing that is what this is; "voice mode is dead behind dario" would have been fixing a bug that did not exist.

  One environment variable redirects it — `VOICE_STREAM_BASE_URL`, which CC reads verbatim through a `process.env[name]` getter with no allowlist of any kind — so it is printed in the startup banner next to `ANTHROPIC_BASE_URL` rather than filed in the docs, because the banner is the only place a user would ever encounter it. dario now answers the upgrade on `/api/ws/speech_to_text/voice_stream`, swaps the `Authorization` header for a pool token, dials `api.anthropic.com:443`, hands back the 101, and then moves bytes. It never parses a WebSocket frame: permessage-deflate, fragmentation, ping/pong and whatever Anthropic adds to the transcript vocabulary next all pass through untouched, and a change upstream does not need a dario release. The price is that dario can see nothing on that socket — no transcripts, no audio, no usage, no rate-limit headers — so a voice socket gets a log line when it closes and deliberately no analytics record, a fabricated zero-token row being worse than an absent one. Two gates stay on CC's side and no server-side change can move them: voice is unavailable when CC resolves its credentials from `ANTHROPIC_API_KEY` or an `apiKeyHelper` (Anthropic's own docs say so, and `isVoiceStreamAvailable` enforces it), and `allow_voice_mode` is a remote gate.

  Three implementation notes worth the space. The outbound leg dials with `node:https`, which looks like a violation of the rule `runtime-fingerprint.ts` and the header of `socks5-bridge.ts` both state — keep TLS on the stack Bun's `fetch` uses. That rule exists to match what CC emits, and CC emits the *inference* path through `fetch` and the *voice* socket through the npm `ws` client, which is `node:https`. Measured by capturing each client's raw ClientHello against a local listener and computing JA3 on Bun 1.4.0: `fetch` is `d871d02c…` (it adds ALPN, status_request and SCT), `node:https` is `71dc8c53…`, and `Bun.connect({tls})` is `203503b7…`, differing from `node:https` in cipher order. All three are the same BoringSSL; only one of them is what CC sends on this socket. Second, both `head` buffers are written before either pipe is wired, because Bun hands post-101 bytes that shared a packet with the 101 to the `head` argument while Node leaves them on the socket — ignore it and the server's first frame is silently eaten on Bun and only on Bun, which is what the coalesced-frame case in `test/voice-ws-relay.mjs` exists to catch. Third, the socket is pinned to the pool's primary via `peek()` rather than routed: the upgrade carries none of the headers `extractSessionAffinitySignals` reads and has no body, so `stickyKey` is structurally null and affinity cannot work here at all — pinning and saying so beats letting `pool.select` scatter voice sockets across seats unrelated to the session doing the talking, and `peek()` keeps it from consuming a round-robin turn that inference routing depends on. A stale token showing up as a 401 is failed over once before the client sees it, because CC classifies any 4xx on this socket as fatal and gives the user no retry of its own. And with `--egress-proxy` configured dario declines the upgrade outright: that flag is implemented by wrapping `globalThis.fetch`, `node:https` does not go through it, and sending one socket direct while every other upstream call tunnels is a worse answer than refusing.

- **A stream that breaks now says so, instead of closing as though it finished.** Once dario has written 200 and an SSE content-type there is no status code left to spend, so every abnormal end of the upstream body — a reset socket, a read timeout, a stream that simply stops — fell through to a bare `res.end()`. That writes a well-formed terminating chunk on a keep-alive connection, which means a truncated response was byte-indistinguishable from a complete one at the HTTP, SSE and transport layers all three. The failure a direct caller would have seen as a transport error came back through the proxy as a clean success. An SDK found out only when its accumulator noticed `message_stop` never arrived (`anthropic-sdk-typescript#842`); anything reading the stream raw never found out at all. The API signals this condition on an already-200 stream with an `error` event — the streaming docs show an `overloaded_error` arriving mid-stream — so dario now says it the same way, `api_error` for a dead socket and `timeout_error` for a stall, and no client needs dario-specific handling to hear it. OpenAI-shape clients get the terminator their SDKs actually block on instead: an error chunk followed by `data: [DONE]`, which is the pair the SSE-overflow path already emitted and the upstream-died path never reached. The frame is written only on the abnormal-exit path, so a healthy stream still reaches the client byte for byte.

  Nothing follows the error frame. Whether `api.anthropic.com` sends a `message_stop` of its own after a mid-stream error is unverified — the streaming docs show the error event in isolation and say nothing about what comes next, and no recording here has caught a real one. Other gateways append one anyway; dario does not, because inventing framing upstream never sent is the class of error this response path exists to remove, and the error event on its own is enough for an SDK to raise a typed failure. A capture of a genuine mid-stream `overloaded_error` would settle it, and the assertion that dario never emits a `message_stop` upstream did not send is what guards the decision until one exists.

  The same cut exposed a quieter loss on the way out. The streaming reverse-mapper swallows every `input_json_delta` for a translated tool block, holding the accumulated JSON until `content_block_stop` licenses one combined delta in the client's own parameter shape. When that stop never came — the stream died, or upstream ended the message with the block still open — the promise went unkept and the arguments were dropped. What reached the client was a `content_block_start` whose input was `{}` followed by a terminal event: a well-formed, complete-looking, entirely wrong tool call. That is worse than an error, because nothing downstream retries it; it reads as the model having called the tool with no arguments. The held blocks are now released as passthrough deltas carrying the raw partial, at `message_delta`/`message_stop` so they land ahead of the terminal event rather than after it, and at `end()` for a stream that stopped without either. Raw rather than translated on purpose: translation is what `content_block_stop` licenses, and absent it there is no ground for calling the JSON complete — the same fallback the 2MB accumulation cap already took. A block that closes properly is untouched, so healthy tool calls still get exactly one translated delta.

- **`/analytics` and `/debug/requests` stop disagreeing about whether a request succeeded.** The streaming record was written with `upstream.status`, and upstream said 200 before it broke, so a truncated stream landed in the rolling window as a success. `/debug/requests` had it right the whole time — `outcome: 'stream-error'` plus the reason — but `errorRate` is the number an operator watches to decide whether upstream is healthy, and it read zero across a run where more than half the streams were cut. Two surfaces answering differently about one request is the failure `test/surface-agreement.mjs` was written for, one layer down. `RequestRecord` now carries `streamTruncated`, set from the same verdict the debug entry records and cleared for a client-side hangup, which is not an upstream failure; `errorRate` counts it alongside the 4xx/5xx statuses. The status field still reports what upstream actually said, so nothing that reads it starts lying in the other direction.

  The same cut hid the burn as well as the failure. Output tokens are read off `message_delta`, which is the one event a truncated stream is guaranteed not to reach, so every cut stream was booked at zero output tokens while upstream had generated and charged for real ones — an under-count biased in a single direction and landing entirely on the traffic that failed, so the worse upstream behaved the less of it `/analytics` saw. The figure is now derived from the characters dario actually forwarded: `text_delta`, `thinking_delta`, and the `partial_json` of tool arguments, at the four-characters-per-token approximation `thinkingTokens` already used. It is an estimate and is marked as one — `outputTokensEstimated` rides on the record, set only when the stream was cut before the real number arrived, so anything reporting exact usage can exclude those records rather than trust them. Content that is not generated output stays out of it: a `web_search_tool_result` payload arrives inside `content_block_start` and is upstream's tool output, not tokens the model produced, and counting it would have inflated a cut web-search turn severalfold.

- **Four more request kinds, recorded rather than reasoned about, and one silent capability loss they exposed.** The first capture covered the shapes one ordinary coding session emits. This one drove real CC v2.1.236 into four it does not: a `Read` on a `.png`, a `Task` that spawns the Explore subagent, a web search, and a turn whose history carries thinking blocks. Each differs from the main loop somewhere that matters. An image is not a top-level content block — it arrives nested inside a `tool_result`'s own `content` array, with the cache breakpoint sitting on the `tool_result` that carries it, so anything walking `msg.content` one level deep does not see it at all. A subagent's billing block has a third field, `cc_is_subagent=true`, and its `system[1]` is the Agent-SDK wording rather than the plain CLI line — both forwarded intact only because the genuine-CC path stopped stamping its own block last release. And the web-search turn declares a *server* tool: a `type` field, no `input_schema`, alongside `tool_choice`, `temperature`, thinking explicitly disabled, no `context_management`, and a beta list carrying neither `claude-code-20250219` nor `extended-cache-ttl` — a sixth distinct list. All four are still recognised as genuine CC, which is the assertion that matters most, since a miss sends them through the template rebuild.

  The finding is on the other path. A tool carrying a `type` field is executed by Anthropic's infrastructure; the client never sees a `tool_use` for it, only the `server_tool_use` and `*_tool_result` blocks the API generates. The name-based mapper exists for client-executed tools and keys `TOOL_MAP` on the lowercase name — which is exactly the shape a server tool's name has. Three of the seven collided: `web_search` was rewritten to `WebSearch`, `web_fetch` to `WebFetch`, `bash` to `Bash`, and for the first two the advertise array grew from the one tool the client declared to thirty-three. The other four survived only because their names happen not to appear in the map. Nothing reported it — `unmappedTools` stayed empty, because as far as the mapper was concerned the tool mapped fine. A client asking for a search Anthropic runs got back a `tool_use` it was expected to run itself. Server tools are now split out before any of that machinery sees them and appended verbatim once the advertise array is assembled. Claude Code was never affected: it sends `web_search` as a server tool too, but it goes down the passthrough path.

- **An error carries an id a client can actually quote, and a different one each time.** Two findings from pointing the official Anthropic SDK at dario, which is a thing no suite had done. The SDK reads `err.requestID` off the `request-id` *response header* — `core/error.js`, `this.requestID = headers?.get('request-id')` — and never out of the body, so dario putting its id in the JSON alone meant every SDK user reporting a proxy-originated failure had `undefined` where the id should be, with no amount of body inspection helping. And the error bodies were built once at `startProxy` time, so every 401 a process served quoted the same `request_id`: two failures sharing an id cannot be told apart in a report. One `sendError` helper now serves all of them — 401, 404, 405, 413, 502, 504 — minting a fresh `req_dario_…` per response and sending it in the body and the header both.

  With that in place the last deliberate divergence goes too: an unknown path now answers `404 {"type":"error","error":{"type":"not_found_error","message":"Not found"}}`, which is what `api.anthropic.com` answers, probed rather than assumed. The 403 it used to send was the truer account of what happened — the SSRF allowlist refusing — but it told a client dario was not the API, and the allowlist decision was only ever interesting to an operator, who still gets it on the verbose log line.

- **Claude Code's wire shape is now pinned to a recording of Claude Code, not to a reading of it.** Every CC-shape assertion in this repo was written from the source and the bundle; this one is written from `test/fixtures/cc-wire-2.1.236/`, a MITM capture of real CC v2.1.236 against `api.anthropic.com` — sandboxed HOME, real credentials, and a TTY session under tmux for the interactive and `/compact` paths. Replaying those captures through the proxy found six divergences that reading had missed, all of them on the byte-faithful path that exists precisely so there would be none. `anthropic-beta` was rebuilt from the template and a per-model transform instead of forwarded: it added `claude-code-20250219` and `advanced-tool-use-2025-11-20`, which 2.1.236 does not send on the main loop, dropped `redact-thinking-2026-02-12`, which it does, and moved `oauth-2025-04-20` from first to eighth. No per-model rule could have been right — across six consecutive requests from one session on one model CC sent five different lists, because it varies them by request *kind*: the main loop carries `extended-cache-ttl`, compaction does not, the title generator carries `structured-outputs`, count_tokens carries four flags total. On the thin count_tokens path the flag list went out as `oauth-2025-04-20,oauth-2025-04-20,…`, a repeat no real client emits. `system[0]`, the billing block, was overwritten with dario's own — built from a template captured with `claude -p`, so an interactive session announced `cc_entrypoint=sdk-cli` in the body while the same request forwarded the client's `user-agent: … (external, cli)`, and the block being overwritten is the one the detector keys on. Every client cache breakpoint was stripped and re-placed, which on the recorded compaction moved CC's single conversation breakpoint off `messages[1]` onto `messages[0]` and `messages[2]` and took the request from three breakpoints to four. `x-client-request-id` was synthesised on every request, and 2.1.236 sends it on none. And `x-stainless-timeout`, which CC sends on `/v1/messages` and omits on count_tokens, was filled in from the template on both — a header the client left out is as much part of its shape as one it sent. All six now forward what CC sent, in the order it sent it. Two fields stay dario's and are asserted to differ: `metadata.user_id`, because the account being billed is dario's, and `x-claude-code-session-id`, because rotation is deliberate. One flag is still added to a forwarded list, and only one: `[1m]` is dario's own model suffix, so a CC pointed at `claude-opus-5[1m]` is asking for the 1M window without sending the flag that enables it — CC has never heard of the suffix. It goes in the slot real CC puts it in, and never on a plain model, where CC does not send it either.

- **Claude Code's quota probe stops being inflated a hundredfold.** CC fires `{"model":…,"max_tokens":1,"messages":[{"role":"user","content":"quota"}],"metadata":{…}}` at `/v1/messages` to read the `anthropic-ratelimit-*` headers off the response — 323 bytes, eight input tokens, no `system` key at all. That last part is why `isGenuineCCClient` read it as foreign: the predicate keys on `system[0]`, and there is no system array to look at. So the probe collected the full template on its way out — 28.5 KB, 55 tool schemas, and `max_tokens` raised from 1 to 64000. It still returned the headers CC wanted, which is why nothing ever surfaced it. The predicate now takes the request headers as a second signal and uses them for exactly this case, the body with no system block: `claude-cli/<version> (external, …)` together with `x-app: cli`, both or neither. The same signal fixes count_tokens, which forwards thin and therefore never reached the code that sets the genuine-CC flag — CC's own `user-agent` was being replaced with the template's there, and `sanitizeMessages` was running its trailing-whitespace trim over the prompt, so an endpoint whose entire job is answering "how big is my prompt" was counting a prompt one byte shorter than the one CC sends.

- **The errors dario answers itself wear the envelope the API uses.** dario's own failures — 401, 403 on an unallowlisted path, 405, 413, the 502 and 504 around upstream — went out as `{"error":"Forbidden","message":"…"}`, a shape with neither `error.type` nor `error.message` where a client looks for them. `api.anthropic.com` returns `{"type":"error","error":{"type":"permission_error","message":"…"},"request_id":"req_…"}`; every Anthropic SDK, Claude Code's included, reads `error.message` to tell the user what happened and falls back to a generic string when it is missing, so a proxy-originated failure arrived with no explanation attached. The status codes are unchanged and `anthropicErrorType` maps each to the type the API uses for it — probed directly for 400, 401, 404, 405 and 413 and taken from `docs.claude.com/en/api/errors` for the rest, which is how the 405 case got caught: the real API answers a wrong method with `invalid_request_error`, not a method-specific type, and the published table does not list 405 at all. `request_id` is stamped `req_dario_…` deliberately — it is the field a client quotes in a bug report, and an id Anthropic's logs have never seen is worse than one that says where it came from. The one divergence this entry left standing on purpose — an unknown path answering 403 rather than the API's 404 — has since been closed; see the entry below.

- **The fixtures can be re-recorded against the installed Claude Code.** `DARIO_LIVE_CC=1 bun test/cc-wire-fidelity.mjs` spawns the real binary against a throwaway MITM and diffs what it sends against the committed captures; the default run is hermetic, so CI needs no credentials, no network and no `claude` on PATH. It pairs each live request with a fixture on top-level key order *and* entrypoint rather than on arrival order: one `claude -p` fires several request kinds in an order that is not stable, and `cc_entrypoint=sdk-cli` and `cli` send different beta lists on identically-shaped bodies — the headless title generator omits `redact-thinking`, the interactive main loop sends it. Matching on either signal alone reports that as drift. A live shape with no fixture is reported as a note rather than a failure, since it means this run exercised something the corpus does not cover, which is a thing to go record.

- **`/api/hello` returns what `api.anthropic.com` returns.** The handler added last release answered `{"message":"Hello, world!"}` with CORS headers, which was a guess. The recording says `{"message":"hello"}`, `content-type: application/json`, no CORS, to both GET and HEAD. `anthropic-workspace-id` joins the forwarded response headers — upstream sends it on every `/v1/messages` response and the CC bundle reads it; dario was dropping it.

- **Claude Code is recognised by the block only Claude Code sends, not by a list of its opening lines.** `isGenuineCCClient` decides the most consequential branch in the request path: a genuine client's bytes are forwarded verbatim, everything else is rebuilt through the template and has its orchestration tags scrubbed first. It required `system[0]` to carry the `x-anthropic-billing-header` block *and* `system[1]` to start with one of five enumerated openers. CC is not five request shapes. The v2.1.236 bundle ships more than twenty distinct system prompts, and of sixteen real ones checked against the predicate, eleven were classified foreign — the code reviewer, the security reviewer, the web-reading specialist, workflow subagents, background observers, the claude-code-guide agent. Each of those runs `sanitizeMessages` over its transcript on the way to being rebuilt from the template rather than forwarded as CC sent it. (An earlier draft of this entry named `/compact` as the worst of them. It was wrong: a recording of a real compaction shows it sends the ordinary main-loop system array with the summariser instruction as the last *user* message, so the opener list matched it. `test/cc-wire-fidelity.mjs` pins that.) Enumerating a moving target was the mistake; the billing block is already the discriminator and no other client emits one. The opener list is gone, and with it the documented custom-agent gap — a `~/.claude/agents` definition puts operator-authored text at `system[1]` with no marker to key on, which is precisely why it used to ride the template path. What the list was also doing, keeping a wrapper client that replays CC's preamble under its own identity off the passthrough path, moves to `detectTextToolClient`, which already knows those clients by name and does not have to be told about CC's next release to keep working.

- **Which path a request took is now written down.** `genuineCCRequest` was computed per request and used only to build headers; it reached no debug entry, no analytics row, no routing trace. A CC release that drifts past the detector does not fail — requests keep returning 200, they are just no longer byte-faithful — so the one signal that would have surfaced the above was the one thing not recorded. The existing `client` field in `/debug/requests` was only ever set for recognised foreign frameworks and left empty for CC; it now carries `cc` or `foreign` for every request. `test/cc-client-fidelity.mjs` pins the predicate against twenty openers taken from the bundle, asserts that what `system[1]` says cannot change the verdict at all — the invariant that stops an allowlist being reintroduced — and, when a Claude Code bundle is installed, reads every `You are …` string out of it and checks that none is misread as a foreign client. Run against the previous predicate, that guard accepts 5 of 58.

- **The response headers Claude Code reads are the ones it gets.** Three inline allowlists, one per response path, each rebuilt from the same four conditions, and they had already come apart: both 429 paths forwarded `retry-after` and the success path did not, and none of the three forwarded `x-should-retry`, so on exactly the responses that header exists to arbitrate the client fell back to guessing. One `isForwardableUpstreamHeader`, called from all three, with `x-should-retry`, `anthropic-organization-id` and `anthropic-usage-limit` added to the rate-limit family and `request-id`.

- **`/api/hello` answers the way the real API answers.** It is Claude Code's connectivity probe — `probeVoiceConnectivity` reads the status verbatim and records `http_403` or `cf_mitigated_*`. `api.anthropic.com` returns 200; dario's SSRF allowlist returned 403 to everything outside its three proxied paths, so every session behind dario reported a blocked probe. It is served locally and never forwarded, so the allowlist is untouched.

- **An account is in the same state on every surface that lists it.** `poolVerdict` settled whether the *pool* can serve; the per-account row was still derived three times. `GET /accounts` — which is what the TUI's Accounts tab renders — decided for itself, `GET /admin/accounts` decided for itself, and neither asked the router. Swept across the 432 combinations of enabled, expiry, auth cool-down, rate-limit cool-down, refresh error and last-known quota status, they disagreed with `select()` five ways. An expired account was listed as `unknown`, the never-measured status, which the TUI prints as no status at all — the row's token column does say `expired`, so the operator is not blind, but the status field said nothing was wrong; one expiring inside `select()`'s 30s slack said nothing *and* rendered its remaining time in green, so a row that cannot serve looked entirely healthy. An expired token that had already provoked its 401 was reported as `auth-cooldown` — the precedence `ineligibleReason` documents at length and `test/pool-eligibility.mjs` asserts, inverted on the one surface an operator actually reads, sending them after a transient blip instead of the credential they have to replace. The admin API was worse: it reported `auth-cooldown` or whatever upstream last said about quota, so `disabled`, `expired` and every cool-down were invisible to a headless operator, and `enabled` was computed for the login-start filter and then dropped from the response — its comment claimed it was "the same snapshot GET /accounts exposes", which is most of why nobody checked. And a cool-down scoped to one model family was flattened to `quota-cooldown`, which reads as fully benched, on an account `select()` was still handing every other model. `accountAvailability` answers the row's question the way `poolVerdict` answers the pool's: `ineligibleReason` first, in its own precedence, then `refresh-failed`, then the quota status. Both surfaces publish `serving` alongside the label, so nothing has to infer routability from a display string again — that is the field the family-scoped case needs, since one global status cannot say "serving opus, benched on sonnet" and `cooldownScopes` already names the family. `refresh-failed` is the one status a serving account may wear: the token works, the router will use it, and renewal is broken, so it ranks below every routing block and above the quota status. The admin snapshot moved out of its closure into `adminAccountSnapshot` for the same reason doctor's line did — it was the only account surface with no test seam. `getToken` keeps a deliberately wider predicate, now documented: it picks a bearer for one `/v1/models` fetch, and a quota cool-down bears on `/v1/messages`.

- **The suite generates the states rather than listing them.** The pool-level disagreement above survived three months of review, and what nobody spots by reading is a surface that agrees on the eight cases someone thought to write down and diverges on the ninth. `test/surface-agreement.mjs` takes the product of six axes and drives each surface for real — an HTTP GET against the route handler, not a call to whichever mapper it uses today — so a surface that stops sharing the predicate fails even when it still typechecks. The invariant is not identical output, which these surfaces have no reason to produce: it is that routability is the router's call, every surface reports that same call, and none names a cause the router did not. A one-account pool is asserted to serve exactly when its account does, which is the seam between this suite and `test/pool-verdict.mjs`. Eight mutations were checked against it, one per finding plus two for drift, and all eight go red — including the case that matters most, both surfaces regressing the *same* way, because the assertions anchor on `ineligibleReason` and not on the two surfaces agreeing with each other.

- **An account's enabled state can be set, not only flipped.** `POST /accounts/:alias/toggle` inverts whatever it finds, and the request carries nothing that says where the caller wanted to end up — so the server cannot tell a retry from a second deliberate press. A client timeout on a request that the server did apply, retried, inverts twice and lands back where it started; a retry that races the original lands opposite to what the operator saw. `POST /accounts/:alias/enabled` takes `{"enabled": true|false}` and puts the account there, which is the same request whether it is the first attempt or the fifth — five concurrent identical sets now all land in the state they named, where five concurrent flips end wherever the parity of the race left them. `enabled` must be a boolean: a coerced body would make the one route whose entire purpose is saying which state you want depend on a conversion the caller cannot see, and `Boolean("false")` is true. Asking for the state an account is already in writes nothing, because reconcile watches those files and a no-op write is churn that reports itself as a change. Toggle stays, for the TUI's single keypress, as a read-then-set under the same per-alias lock — the TUI itself now sends the explicit state, since it knows which row the operator is looking at.

- **`t` on the Accounts tab is exact, like every other key on it.** It was matched case-insensitively — the only one that was — so shift-T returned a deliberately benched account to routing, and to billing, in one keystroke. `d` and `x` are exact *and* put a y/n confirm in the way; `t` had neither.

- **One pool, one answer to whether it can serve.** Four surfaces report on the account pool — `dario doctor`, the 503 body, `/health`, and the startup banner — and each filtered the accounts itself, so one pool got three answers. Measured against a pool of two disabled seats: the router returns null and every request 503s, `/health` says `broken`, the banner says `healthy — 2h 0m`, and the client is told "all accounts are rate-limited or in auth cool-down; retry shortly", naming two causes that are not the cause and promising a recovery that will never happen without the operator. Against a pool of expired tokens the same three disagree in the opposite direction: the banner says `expired`, `/health` says `healthy`, on the stated grounds that background refresh will roll it — refresh runs every 60s against a 45-minute margin, so a token expired *now* is one refresh has already failed to roll. `/health` also reported healthy for an all-rate-limited pool, which was never consistent with the all-auth-cooldown case reporting broken. `ineligibleReason` is the predicate requests actually pass through and it already answers this; `poolVerdict` asks it once and the four surfaces render the result, differing in wording and not in verdict. `blockedSummary` gives them the same words for the same state, so "the only account is disabled" is what the operator reads from all three. Two surfaces keep a deliberate local decision, and only one each: the banner still reports `expired` rather than `broken`, because `dario login` is the answer to a dead token and not to a benched seat, and the 503 body appends "retry shortly" only when every cause in play clears on its own. `/health` newly 503s for an expired or all-rate-limited pool, which is the contract it advertises — it 503s when OAuth is in a state that fails every upstream request — with one case where it now calls the proxy dead while something still gets through: an exhausted pool serving OpenAI-shape clients through the fallback backend. Anthropic-shape requests, which are the reason dario exists, still 503 there.

- **Doctor's `disabled` routing message was unreachable the day after it was written.** `74ae807` added a branch naming the account `select()` had returned and why it was blocked, because `select()` then fell back to an ineligible account when nothing was eligible. `1c4bd44` removed that fallback the next day, so from then on `select()` returned an eligible account or null and the branch could only ever print its no-account half — the `disabled` and `auth-cooldown` messages it carried were dead, and the comment above them explained a behaviour that no longer existed. The same stale claim sat in `ineligibleReason`'s own doc comment. The rendering moved out of `runChecks` into `poolRoutingCheck`, which is pure and takes the verdict, so the line is testable at all: a mutation that reverts it to the old single generic cause is now caught by name.

- **No suite can reach the operator's home directory.** 165 of the 176 suites set no `HOME` override of their own, so they ran against the real one. Measured by keeping the sandbox roots instead of sweeping them, ten of them write into `~/.dario`: five produce `accounts/login.json` — a routable seat, in the operator's live pool, holding a token that cannot authenticate — and five overwrite `cc-oauth-cache-v6.json`, which is their actual CC OAuth cache. The credentials are synthetic, checked against the real store by hash with no match on token or device id, so nothing leaked; the traffic went the other way, into live state, on every test run. Both are cleaned up on the happy path and only there — a crash or a failing assertion leaves them behind, and reconcile does not care that a test wrote them. The driver now gives each child its own `HOME` (and `USERPROFILE`, which is what `os.homedir()` reads on win32), one per suite rather than one shared: three to eight run at a time and the account store is keyed by alias, so a shared home would have them writing each other's credential files. Fixing it at the driver is the only version that stays true as suites are added — patching the ten leaves the 165th free to become the eleventh. All 176 pass against it, so nothing in the tree actually wanted the operator's home.

- **`dist/` is checked against the `src/` next to it.** The suite is split on which tree it reads — text assertions grep `src/*.ts`, behavioural ones import `dist/*.js` — and nothing checked that the two describe the same program. A source change that was never compiled shows green text assertions on the new code and green behavioural assertions on the old, which is not hypothetical: a mutation sweep in this tree produced exactly that state. It is worse than the usual staleness case on a machine where `dist/` is a symlink to a build directory shared between checkouts, because "stale" then also means "another tree built last, and every behavioural assertion in this run tested that tree". `bun run build` ends by stamping a hash of `src/` plus `tsconfig.json` — tsconfig because a target or module change alters every emitted file without touching a source byte — and `test/dist-matches-src.mjs` recomputes it. A hash rather than mtimes, since mtimes pass whenever the other tree built more recently, which is precisely the shared-directory case. The stamper and the check share one hashing implementation on purpose: two would drift, and a correspondence check that has drifted from what it checks reports agreement it never verified. A missing `dist/` and a `tsc` that emitted nothing are both failures with their own message rather than a stack trace.

- **A test that could not fail now measures what it claimed.** `test/request-debug.mjs` asserted `8192 * 6 * 2 > 32_768` — an inequality over three constants, true at parse time, touching neither the store nor the record. It stood for the premise that JSON-escaping a control-character preview inflates it past `MAX_LINE_BYTES`, which is the whole reason the write cap exists; it is now computed from the serialized record itself, and goes red when the premise stops holding.

- **A live capture the bundle beats outright is refused rather than merged into.** Read-time repair covers two of the three axes: `withBundledFallbacks` restores the bundle's tools on the bundle's ordering and merges its prompt variants, so a narrow capture costs nothing there. Nothing repairs `system_prompt` — it is the one thing only a capture can supply — so a degenerate one was served for the cache's full 24h TTL with no route back short of deleting the file by hand. The gate is structural, not a threshold. An empty prompt, an empty identity block, a prompt byte-equal to the identity, or a prompt no longer than the identity all say the same thing: the captured system blocks are not in the three-block layout `extractTemplate` assumes, and block [2] holding the billing tag or a truncated read lands in exactly that shape. One identity check joins them — a capture sharing not one tool name with the bundle did not come from a CC this dario knows. What the gate deliberately does not do is reject on tool count, because a `claude --print` capture is narrower by construction: 12 tools against the bundle's 34 on this host, ten of them shared, which is a headless CC being headless and not a defect. Rejecting on count would reject every capture on every machine and throw away the prompt to protect the axis that already has carry-forward. A size floor relative to the bundle was the first attempt and was wrong for the same reason from the other end — the legitimate spread across model families is 5.7x on this bundle alone (4802 bytes scrubbed base to 27594 live haiku), so any ratio tight enough to add something sits inside the range of correct prompts; what it caught immediately was three suites' synthetic fixtures. The check runs on the write path before the variant sweep, not after, since the sweep is four more captures and there is no point paying for them to decorate a template about to be discarded, and on the read path in `loadTemplate`, which is what covers a degenerate cache written by an older dario or by hand. `withBundledFallbacks` stays a pure merge and judges nothing — folding the refusal in there broke four suites that build four-byte templates to exercise the merge itself, which is a perfectly good input to a merge and a terrible thing to serve, and only one of those two functions has any business saying so.

- **The startup banner attributes each axis instead of reporting the merged totals as captured.** It read `live capture, CC v2.1.235 (3m old), variants: fable+opus-5+sonnet-5, tools: 34` off a cache that carried no variants at all and ten of those thirty-four tools. Every word after "live capture" described the bundle, and the one axis it named by content was the axis with carry-forward protection — which is how a halved tool list read as healthy right through a release. The merge now stamps what it had to supply, and the line says so: `variants: fable+opus-5+sonnet-5 (3/3 bundled), tools: 34 (24 bundled)`. Both parentheticals are suppressed when the bundle contributed nothing, so a healthy line stays as short as it was. The stamp is derived and never persisted — the cache file remains the honest record of what was captured, and the union is recomputed against whatever bundle is shipped at read time, which is what lets a bundle that drops a tool actually drop it. Writing the union back into the cache would make every dropped tool immortal.

- **The suite runs with a live template cache present, which is the configuration every operator with CC installed is in.** The driver exported `DARIO_LIVE_TEMPLATE_CACHE` pointed at a path that does not exist, so all 175 suites ran against the bundle and the entire live-capture read path — the merge, the fallbacks, the banner, the TTL comparison — was exercised only by the handful of suites that built their own cache. The narrowing fixed above lived in that blind spot for a full release. Children now get a fixture derived from the shipped bundle at run time rather than a committed 132 KB blob: the bundle's own tool definitions filtered to the twelve a headless `claude --print` actually returns, which leaves ten because two of them the bundle does not carry, no variants, `_source: 'live'`, captured now. Three suites that assert on interactive-only tools pass against it because the read-time union restores them, which is the point. Alongside it, a `DARIO_CLAUDE_BIN` stub harness drives the capture end to end for the first time — a generated script that answers as CC and one that never sends — and asserts on what the child actually received and on the filesystem after settle. Ten semantic mutations of the capture path used to leave the suite green because none of it ran; the harness now catches the child-environment allowlist collapsing back to `process.env` by naming all hundred variables that leak through it, including the operator's API keys and `SSH_AUTH_SOCK`, which nothing before could see. The stub carries its scenario in its own source rather than an environment variable, deliberately: a scenario that arrived would mean the allowlist had a hole.

- **The capture child's environment is an allowlist, not everything-minus-four.** The spawn inherited `process.env` and deleted `ANTHROPIC_AUTH_TOKEN`, `CLAUDE_CODE_USE_BEDROCK` and `CLAUDE_CODE_USE_VERTEX` from it. A denylist cannot hold: CC reads more redirect variables than any list here will name, and each one fails the same silent way — the child goes somewhere other than the MITM, bills the operator, and capture reports "no request arrived", which is also what a machine with no CC installed reports. Measured against the installed binary, all of these reached a real spawned child on top of the deleted four: five more `CLAUDE_CODE_USE_*` platform switches with their matching `ANTHROPIC_*_BASE_URL` pairs, `ANTHROPIC_API_HOST`, `ANTHROPIC_UNIX_SOCKET` which bypasses the base URL entirely, `ANTHROPIC_CUSTOM_HEADERS` which also pollutes the captured header order, `CLAUDE_ENV_FILE` — CC reads that file and applies it as the session environment, reopening the original bug verbatim — and all six proxy variables, which matter because the MITM is plain http on loopback, so a host with `HTTP_PROXY` set and no loopback entry in `NO_PROXY` hands the capture request to that proxy, a population that overlaps heavily with the router operators this sandbox exists for. The child now gets enough to find and run a binary and nothing that can decide where its request goes. Verified by capture rather than by reading: a child spawned with exactly this set produced a system prompt byte-identical to one spawned with the full 104-variable environment, on every family in `VARIANT_FAMILIES`. The counterfactual holds too — with `HTTPS_PROXY` pointed at a dead port the previous build captures nothing and prints the billed-or-nothing warning, where this one completes the five-capture sweep. The API key handed to the child is always the placeholder now; inheriting the operator's real one bought nothing, since the MITM authenticates nothing, and put a live credential in a child deliberately pointed at a socket.

- **The managed-settings bail checks the other seventeen keys.** `CLAUDE_CONFIG_DIR` cannot relocate a machine-level policy file, so the capture declines to spend the operator's subscription discovering one — but the check read `env.ANTHROPIC_BASE_URL` and nothing else. Exercised against the real function with injected policy files, it proceeded on every other way a policy file can move the child. `CLAUDE_CODE_USE_BEDROCK` and `_VERTEX` were the sharpest: the spawn deleted both from the child's environment *because* it knew they were billing routes, and named them in its own warning text, while the guard waved them through. `apiKeyHelper` is the subtle one — the binary maps it to `["ANTHROPIC_BASE_URL", "_CLAUDE_CODE_ASSUME_FIRST_PARTY_BASE_URL"]`, so it set the very variable the guard checked, by a route the guard could not see. `model` is on the list for a different reason: it does not bill anything, it defeats the `ANTHROPIC_MODEL` pin, so every family in the variant sweep would capture the same wrong prompt and dario would serve it to the others — a capture that is silently the wrong model is worse than no capture, because the bundle is at least right about which model it is. The log line names the key it found rather than asserting which one it must have been.

- **Every model family gets its own captured prompt, and haiku gets one at all.** The runtime refresh captured once, on the base model, and wrote a cache with no `system_prompt_variants`; the bundle's were merged in as a fallback, which is the right failure mode but not a mirror — a bundled variant is scrubbed of host context and as old as the last bake, so anything measurable only under a given model was simply unavailable. The sweep captures a prompt per family, at one `claude --print` each against the loopback MITM, which bills nothing and runs in the background refresh rather than on a request path. What it recovers is concrete: opus-5's real knowledge cutoff is May 2026 where the base says January 2026, and haiku's is February 2025. Haiku was the larger half, and was missing from `VARIANT_FAMILIES` entirely — CC sends it the long-form prompt, 27.7K against the base's 6.2K, and dario served it the base. A 21K divergence on a first-class model with no wire symptom at all. The bundle cannot gain the variant in the same commit: only `capture-and-bake` can write it, and that script also stamps `_supportedMaxTested`, which the cc-drift bot owns. `awaitingFirstBake` licenses that one absence, and the invariant suite fails once the bake lands — naming the line to delete — so the flag cannot outlive its reason.

- **The `gitStatus:` block CC sends from inside a working tree is sent.** CC appends branch, PR-target branch, git user, `git status --porcelain` and five commits after the last heading whenever it is invoked from a repository. dario sent none, because the capture ran in a bare `/tmp` directory where CC emits nothing — no captured shape, and this codebase does not invent one. The capture sandbox is a git repository now, seeded clean and fully isolated from the operator's git config, so there is a shape to rewrite; a global `init.templateDir` or `core.hooksPath` would otherwise have run inside the capture. Every empty case resolves by the rule the environment rewrite already follows. A clean tree keeps CC's own `(clean)` — which is why the sandbox is seeded clean rather than left dirty. An unset `user.name` drops its own section and leaves the rest standing. A repository with no commits drops the commit list rather than restating the capture's. A host outside a repository has the block removed, which is what CC sends there. Each of those shapes was measured against CC 2.1.236, including the PR-target rule: `main` when the repo has one, `master` when it has only that, and `main` even on a repo with neither. One cost, named rather than hidden — the system prompt ships `cache_control: ephemeral`, so a gitStatus that moves invalidates a ~25KB cached prefix; it only moves when the repository actually changes, and the snapshot TTL matches the cache's own five-minute lifetime, which bounds it to at most one extra miss per cache generation.

- **The capture sandbox's own config path no longer drifts the bundle.** The capture relocates `CLAUDE_CONFIG_DIR` into a random `dario-capture-XXXXXX` temp dir, so CC composes its memory path under that rather than under `~/.claude` — a root the canonicalizer does not know and the identity rules never see, because a temp dir carries no username. Left alone it is #854 in a new shape: the sandbox name is fresh per capture, so every bake writes a different byte string for the same logical path, every check calls it drift, the auto-rebake loop reopens, and the published bundle carries a path out of the bake host's `/tmp`. It has not reached a bundle only because the sandbox landed after the last bake. Collapsed to the same placeholder a real install's path gets, `--check` now reports the same -51 delta for the base and all three variants — real CC 2.1.235 → 2.1.236 drift, and nothing else.

- **The `# Environment` section is rendered from the machine actually serving the request.** A live capture runs `claude` in a throwaway config home, so the block it records describes that sandbox — and dario replayed it verbatim. Every model outside the three prompt variants was told its working directory was `/tmp/dario-capture-cGZjcC`, a path that stops existing when the capture ends, that git was unavailable, and that it was Opus 4.8; a haiku request got all three. The variants had the opposite problem: the bake scrubs host context before publishing, correctly, since the bundle ships to other people, so an opus-5, sonnet-5 or fable request carried no environment section at all where real CC always sends one. Both halves are answered at request time now, for every model. The rule is rewrite, never invent — host and model lines are replaced from facts read off the running system, and every line CC authored (the model catalog, the availability blurb, the fast-mode note) passes through untouched, because dario has no source for that prose other than the binary that produced it. A fact that cannot be sourced leaves the captured line standing; a line that is model-specific and unsourceable is dropped rather than restated, which is why the capture's "January 2026" cutoff — opus-4-8's answer — does not follow the request onto opus-5. A missing line beats a false one, and the shape delta costs nothing: system prompt content and length are not billing-classifier inputs. An install that has never run a live capture has no block to model and is left exactly as it is. The section goes immediately before `# Context management`, where CC puts it, verified against two independently produced prompts. The operator's real working directory now goes upstream where a sandbox path went before — a deliberate trade, a 1:1 mirror of real CC over the incidental redaction a wrong path provided.

- **A disabled account is no longer reported as spent quota.** `pool.status()` derived `exhausted` as `accounts - healthy`, and `healthy` is gated on `ineligibleReason`, which reports `disabled` ahead of every other cause. So switching a seat off moved it straight into the quota bucket: on this machine, toggling an account with a valid 7.8 h token and an empty rate-limit window took `healthy 2→1, exhausted 1→2` and told the operator their own deliberate act was exhaustion. The two causes are counted apart now — `PoolStatus` carries `disabled`, `exhausted` means enabled-but-unable, and `healthy + exhausted + disabled === accounts` holds including for the seat that is disabled *and* expired, which the old arithmetic was happy to count twice. `doctor` consumed the field as the total unavailable, so it adds the two back and names the benched seats instead of filing them under cool-down. The averaged `totalHeadroom` on the adjacent line already excluded disabled accounts; that half was right, and is now pinned.

- **The distributed refresh lock stops believing whatever the lock service says.** `lockCall` ended `return (await res.json()) as T` — an assertion on a value that had just crossed the network, read by two call sites with no guard. `acquired` is tested for truthiness, so a body carrying the *string* `"false"` claimed a lease this instance never held, refreshed under it, and released a lock belonging to someone else: the exact race the lock exists to prevent, reached through the lock. `retryAfterMs` reaches `Math.min(x, 3_000)`, so a string or a null made `sleep()` resolve on the next tick and burned all eight attempts in one — a backoff that does not back off. A malformed reply is now a lock-service fault, and a lock-service fault fails open exactly as an unreachable one does, which is what the credentials guard one branch below already did. A non-object body is reported as no reply rather than cast into one, and `isLockAcquireResult` checks `acquired` and the optional `retryAfterMs` before either is read.

- **The unmapped-tool round-robin no longer claims a substitution it cannot make.** When a client declares a tool dario has no mapping for, the arm assigns it a CC "fallback slot" chosen from `CC_FALLBACK_TOOLS` with the tools the client already claimed filtered *out*, so two client names never collide on one CC tool. The advertise path emits only the tools the client *did* declare. On an ordinary mixed client those two sets are exact complements, so the slot the arm picks is never in the outgoing array — and `[dario] tool substitution: N/M client tools … remapped onto CC fallback slots` said otherwise on every such request. Verified: a client declaring `Read`, `Bash` and `memory_get` advertises `["Bash","Read"]`, routes `memory_get` onto `Grep`, and does not advertise `Grep`. The mapping is not inert, which is the part that had gone unrecorded — the history remap runs regardless of what is advertised, so a past `tool_use` for `memory_get` *is* renamed to `Grep` and its input `{key:"user_prefs"}` replaced with `{pattern:".",path:"."}`. The model reads a transcript in which it called a tool absent from its own tool list, with the argument destroyed. `buildCCRequest` now returns `unreachableTools`, reconciled against the finalized array rather than assumed — the no-declaration fallback, merge mode and fable's pinned array all send the whole template, where every slot *is* advertised and the arm works as documented — and the startup line reports the loss instead of implying a substitution. Behaviour on the wire is unchanged: advertising the chosen slot would make the rename land, but it re-advertises a CC tool the client never declared, which is exactly the failure `e409f52` exists to prevent. One code path, two client classes, opposite requirements; the drop is now visible rather than silent.

- **`startProxy` reports its bind instead of exiting on the caller's behalf.** It returned `Promise<void>` and called `process.exit` from the server's `error` handler: 1 for a foreign occupant, and — the damaging case — **0** whenever the occupant answered `/health` as a healthy dario, on the reasoning that a human running `dario proxy` twice wants a no-op rather than a crash. That reasoning holds for a human and fails for every other caller. The standalone test runner scores a suite on its exit code, so a suite whose hardcoded port was held by the developer's own running dario bound nothing, asserted nothing, exited 0, and was counted as passed; eight suites could reach that state. It now resolves to a handle carrying the port the kernel actually assigned and rejects with `ProxyBindError` — `darioAlreadyRunning` and the `/health` fields the old banner printed ride along, and `cli.ts` rebuilds that banner and its exit 0 from them, so the interactive behaviour is byte-identical. Verified A/B against an occupied port: the same suite exits 0 having run nothing before, and 1 after. Four suites that hardcoded ports (39873, 38761, 38773, 38781) now pass `port: 0` and read the assignment back, and `session-affinity-proxy`'s hand-rolled `freePort()` — which bound a probe server purely to read a port off it and close it again, narrowing the race without closing it — is retired for the same reason. `close()` on the handle releases the socket and both intervals without touching process lifetime.

- **The test runner no longer scores a suite on its exit code alone.** Exit 0 is not evidence a suite ran, and the `startProxy` fix above removes only one route to proving it. A suite must now also report a countable tally; one that exits 0 having made zero assertions, or printing nothing a tally can be read from, is failed on the spot. The suites predate any shared reporter and each prints its own summary, so the reader takes the six spellings actually in the tree rather than imposing a seventh. Suite discovery moved beside it so anything reasoning about "every suite" agrees with the driver instead of re-deriving the list.

- **A live capture no longer narrows the bundled tool union.** The bundle is deliberately a superset, and three registries in `cc-template.ts` exist only to keep it one — `PLATFORM_ONLY_TOOLS`, `INTERACTIVE_ONLY_TOOLS`, `CONFIG_SCOPED_TOOLS` — each merged forward by `capture-and-bake.mjs` so a headless re-bake cannot drop them. None of it protected the runtime: `cc-template.ts` derives `CC_TOOL_DEFINITIONS` and `CC_NATIVE_NAMES_UNION` from `loadTemplate()`, which prefers a live cache under 24h and passed its `tools` through untouched — so the narrowing the bake defends against happened anyway, on every machine with CC installed. Measured: the cache held 24 tools against the bundle's 34, and the missing ten were exactly those three sets with no remainder, while the 24 they shared were byte-identical and in the same order — the cache contributed nothing and cost ten tools. The cost is the v4.8.93 regression, where a client declaring a dropped tool stops identity-mapping and falls into the unmapped round-robin with junk args. The union rebuilds on the bundle's ordering rather than appending, because the dropped tools sit at interior indices and `buildCCRequest` writes template order onto the wire verbatim when the client declares nothing; a live definition still wins where both carry the tool, and a tool only the live capture has is kept — spliced at its alphabetical position, not appended, because CC sends tools sorted by name and the bundle is strictly ordered that way (`capture-and-bake.mjs` sorts after every preservation merge for the same reason). Appending would have emitted an order no real CC sends, in exactly the branch that arm exists to serve: a CC newer than the last bake. `tool_names` is re-derived, since updating `tools` alone is the divergence class that once shipped a bundle with `tool_names` 30 against `tools` 33.

- **The session presence heartbeat no longer pulses with a disabled account.** It took `pool.all()[0]`, and `all()` returns every configured account including benched ones — disabling is a routing decision, not a removal — in reconcile insertion order, which says nothing about whether a seat may serve. On a pool whose first account is disabled, the proxy kept POSTing `/v1/code/sessions/<id>/client/presence` with that seat's bearer token every 5s: exactly the traffic the operator benched it to stop, and invisible, because the pulse is out of band and never reaches the request log. Every other `pool.all()` consumer already filtered on `enabled`; this one was correct only by the order accounts happened to load in.

- **The live-capture config dir is swept when the child has already exited.** Cleanup was armed on the child's `exit` event behind a guard that the child was still running, so a `settle` that ran after the child was gone swept nothing at all: the guard was false, `kill` was a no-op, and the `if (!child)` fallback was false because the child object still existed. That path is guaranteed whenever the capture fails — a child that exits without sending settles from its own `exit` handler — which is the path that repeats once per proxy start for as long as capture stays broken, and it is racy on success besides, where a 500ms request-arrived timer and a 200ms child-exited timer decide it. 19 stranded dirs had accumulated on one machine, each holding a full `.claude.json` and a session transcript. Verified A/B against a stub binary that exits without sending: one stranded dir per run before, none after. The opposite arm leaked too, and for the mirror-image reason: everything that armed cleanup was attached to the *child*, and all of it is cancelled by the *parent* exiting first — the `exit` event is never delivered, and the 30s backstop is `unref`'d precisely so it cannot hold the process open. Every dario invocation shorter than its own capture leaked a dir that way: `doctor`, `--version`, any CLI path that arms the background refresh and returns in milliseconds, and every test that starts a proxy without `noLiveCapture`. The dir is now registered for sweeping at `mkdtemp` rather than inside `settle`, which is what covers the whole window — a parent exiting before `settle` ever ran had no handler armed at all — with a process-exit hook as the backstop, armed once rather than per capture, and deregistered by a normal sweep so a long-lived proxy accumulates nothing.

- **A refresh error no longer outlives the token it was raised against.** `pool.add` carried `refreshError` forward unconditionally and reconcile re-adds every account from disk on every run, so an account refreshed by another dario instance — or re-authed out of band — kept reporting `refresh-failed` while holding a working token. Nothing on the reconcile path clears the message, so only a refresh through the proxy's own retry loop ever could. It is now preserved only while the access token is unchanged. The Accounts tab had the matching ordering bug: `refresh-failed` outranked `auth-cooldown`, so a stale message hid the live cooldown that is the account's actual reason for being ineligible. Cooldowns now win, matching `ineligibleReason`; the error is still shown in its own field.

- **The distributed refresh lock's adopt path no longer overwrites local state.** When another instance has already refreshed an alias, dario adopts that instance's credentials instead of calling Anthropic. It took the record whole — but `enabled`, `deviceId`, `accountUuid` and `scopes` are local state, so adopting resurrected a locally disabled account and replaced this install's device identity with the peer's. The alias came from the response too, and the alias names the file being written: the runtime guard on the storage boundary was called without its `expectedAlias` argument, so a response naming a different account was accepted and clobbered it. Only the token triple is adopted now, spread over the local record the way `doRefreshAccountToken` always has, and a response that fails the alias check falls open to a real refresh exactly as an unreachable lock does. Reachable only with `DARIO_REFRESH_LOCK_URL` set.

- **The request-diagnostics write queue is bounded, and a failing write backs off.** `mkdir(dirname)` sits before the drain loop, so a failure there — a debug log pointed at a directory that cannot be created — returned with the queue untouched and the next request pushed onto it again. `entries` is a ring and stayed flat; the queue grew without bound. It is now capped at `COMPACT_FACTOR × maxEntries`, the file's own bound rather than the ring's, so the surplus that keeps appends O(1) is still allowed. Separately, every request re-armed the write, and `persistedLines` is only lowered by a successful rewrite — so once past the compaction threshold a failing rewrite kept the store past it, and every subsequent request paid a full O(`maxEntries`) serialization instead of an append, permanently, while I/O was the broken thing. Failures now back off from 1s to a 5m ceiling and reset on the first success. `flush()` no longer forces a drain while paused, so a shutdown inside a backoff window drops the queued entries — they were unwritable anyway.

- **Live fingerprint capture no longer bills a real request.** The capture spawns `claude --print -p 'hi'` against a loopback MITM that answers with a canned SSE stream, so it is supposed to cost nothing. CC's `settings.json` `env` block takes precedence over the environment handed to the child, so on any machine that pins `ANTHROPIC_BASE_URL` there — the normal setup behind a router proxy — the spawned CC ignored the MITM and went upstream for real, and the `ANTHROPIC_MODEL` pin was overridden by `ANTHROPIC_DEFAULT_*` the same way. The failure was silent in both directions: the operator paid ~22K tokens per proxy start against their subscription, and capture reported `CC did not send a /v1/messages request`, which is also what a machine with no CC installed reports — so `cc-template.live.json` was never written and the proxy served the bundled template forever while paying for a live one. The spawn now relocates `CLAUDE_CONFIG_DIR` to a throwaway dir so no `settings.json` is in scope, drops an inherited `ANTHROPIC_AUTH_TOKEN`, and pins `cwd` there too — without it the child inherited the proxy's working directory and dragged that repo's `CLAUDE.md` and git state into a template meant to be generic. The dir is removed when the capture settles; CC treats it as its home, so leaving them behind was filling `~/.claude/projects` with one junk `hi` session per proxy start. That removal did not cover a child that had already exited — see the sweep entry above, which is what makes this claim hold on the failing path.

- **Request-diagnostics persistence stops rewriting the whole window on every request.** The append path required `persistedLines < maxEntries`, so once the ring filled every persist fell through to a full rewrite — `JSON.stringify` over all 4096 records, a tmp file, and a rename, per request. The raised default (512 → 4096) made that steady state ~8× more expensive rather than buying a larger window for free. The file now carries surplus lines up to twice the window and compacts in one pass when it crosses, so the O(n) rewrite is paid once per `maxEntries` requests; `load()` already kept only the newest records, so nothing changes on reload. Measured at the observed 9.2 KB/record with a 4096-entry ring: 132 ms per request → 0.15 ms, and 37.7 MB of writes per request → ~18 KB — an append plus the amortized share of the rewrite, which lands once per `maxEntries` requests and costs another whole record's worth. The synchronous serialization is reduced in frequency, not removed: the request that crosses the compaction threshold still serializes the full window and stalls ~50 ms, it just does so once per `maxEntries` requests instead of on every one. `MAX_LINE_BYTES` is now enforced on write as well — it was declared but only ever used to size the reload window, so a line that overran it silently pushed older records out of that window on the next boot.

- **Re-login no longer silently re-enables a disabled account.** A replacement login rebuilds the credential record from the OAuth exchange, which carries no `enabled` field, and a missing field loads as enabled — so re-authenticating a benched account put it back into rotation and billing with no sign anything had happened. `replaceAccount` now carries the stored value forward inside the alias lock, where it is atomic against a concurrent toggle; an explicit `enabled` on the incoming record still wins. `POST /admin/login/start-needed` made this self-driving: it selects on `consecutiveAuthFailures` alone, and a disabled account serves no traffic so nothing ever resets that counter — an account disabled *because* it kept 401ing was advertised as needing login forever, and completing that login re-armed it. Disabled accounts are now excluded from the candidate list.

- **Accounts can be disabled without being deleted.** A new `enabled` flag on each account's credential file (`~/.dario/accounts/<alias>.json`) takes an account out of the rotation while keeping it in the pool — `POST /accounts/:alias/toggle` (loopback-only) or the `t` key on the Accounts tab. Disabled accounts are skipped by routing, startup and background token refresh, plan probing, and quota fetches, but stay listed. A missing `enabled` field loads as enabled, so existing accounts are unaffected by the upgrade.

- **Token-refresh failures are visible and back off.** A malformed token response (missing `access_token`/`expires_in`) is now a hard refresh error rather than saving a corrupt record. The last refresh failure per account is recorded on the pool and shown in the Accounts tab as a `refresh-failed` status with the error message; a successful refresh clears it. The background refresh loop moves from a fixed 15-minute tick to a 1-minute tick with per-account exponential backoff (1min base, 30min cap), so a persistently failing account stops hammering the token endpoint and a re-enabled account retries immediately.

- **Request-diagnostics retention is configurable up to 4096 entries.** The bounded redacted request log that feeds `/debug/requests` and the Hits inspector keeps 4096 records by default (was 512) and accepts `DARIO_DEBUG_LOG_LIMIT` up to 16384, so a hot retry loop no longer evicts the routing history needed to diagnose it. Persistence across restarts (append-on-write, reload-on-boot, mode `0600`) is unchanged.

- **Request diagnostics now expose upstream dispatch attempts.** Hits detail panes report whether a client request completed on its first upstream call or required Dario recovery/failover, making token-burn investigations distinguish proxy retries from client fan-out. The managed service also retains the newest 4096 redacted request records in a FIFO JSON-ND log and exposes them through the loopback-only `/debug/requests` endpoint; records include token/cache accounting, body and affinity fingerprints, initial/final account, and explicit retry reasons without storing prompt content. Override persistence with `DARIO_DEBUG_LOG_FILE`, the bound with `DARIO_DEBUG_LOG_LIMIT`, or disable it with `DARIO_DEBUG_LOG=0`. Structured auth errors returned with HTTP 400/429 now participate in pool cooldown/failover, and replayed passthrough bodies no longer retain a stale `Content-Length` header.

- **Hits request inspection is stable and agent-readable.** Inspector scrolling is clamped to the rendered viewport, including `PageUp`/`PageDown`, `Home`, and `End`, so moving Up works immediately after reaching the bottom. Live SSE arrivals no longer replace the record under inspection. Arbitrary Unicode previews now wrap by terminal-cell width, terminal control bytes are neutralized, and authenticated remote analytics subscribers receive metadata without request/response text. [`docs/agent-diagnostics.md`](docs/agent-diagnostics.md) documents the TUI, FIFO request log, routing trace, field semantics, privacy boundaries, and repeatable investigation workflows for coding agents.
...
- **The wire-fidelity suites are now checked against the bug they claim to catch.** A suite that passes proves nothing on its own — it passes just as happily against code that never had the bug, and the only evidence an assertion is load-bearing is watching it go red when the behaviour is taken away. That was done by hand once, when the `cc-wire-2.1.236` corpus landed: six reverts, six catching assertions, and nothing that would notice when the seventh assertion quietly stopped asserting. `scripts/mutation-gate.mjs` (`bun run test:mutation`, opt-in, deliberately outside `test/` because the runner discovers every `test/*.mjs`) does it repeatably for twenty-three of the recent fixes. Each entry is a literal string swap in the built output plus the suite that must fail and the assertion LABEL that must be the one to do it; matching on the label rather than the exit code is the part that matters, since a mutation that fails a suite by crashing its setup says nothing about whether the behaviour is pinned. Three outcomes are failures of the gate: the suite passed (the assertion is tautological or absent), it failed somewhere else (the coverage is incidental), or the search string is no longer in the build (the entry is stale, which is exactly what rots a hand-run mutation list into decoration). It never touches the real `dist/`, which is a symlink to a build directory shared with a running service and with other test lanes: suites resolve `'../dist/proxy.js'` relative to their own file and Bun resolves through symlinks, so there is no import-specifier or `NODE_PATH` seam to hook — `dist/`, `test/` and `scripts/` are copied to a scratch root and the suite is run from there, which lands `../dist` on the copy by construction. An entry may name more than one assertion, in which case every one of them has to go red — a revert cannot be credited to a label that stayed green. All twenty-three are caught: the identity-header forgery check, the quota probe's header fallback, the opener allowlist, the billing block, `metadata.user_id`, breakpoint placement and forced TTL, top-level key order, `dedupeBetaFlags`, `anthropicErrorType`, `sendError`'s per-response `request_id` in both the body and the `request-id` header, the 404 an unknown path now answers, `x-should-retry`, `/api/hello`'s body, the forwarded beta list and the one `[1m]` flag added to it, `x-client-request-id`, the omitted `x-stainless-timeout`, count_tokens' identity headers, the trailing newline `sanitizeMessages` used to take off the prompt being counted, and the server-tool split that kept `web_search`, `web_fetch` and `bash` out of the name mapper.

- **`account-refresh-singleflight` stops racing a binary scan.** It failed once under `TEST_CONCURRENCY=4` and passed standalone, which is the shape of a flake nobody chases. `waitForFetches` gives the first refresh POST five seconds to leave, and `doRefreshAccountToken` awaits `detectCCOAuthConfig` on the way there — which on a cold cache reads and scans the entire installed Claude Code binary, 319 MB of it. The runner hands every suite a HOME of its own, so `~/.dario`'s cache is cold on every run and that scan is inside the budget every time: ~1.4s idle, 4-5s across twelve concurrent processes, over 20s in a burst. Reproduced twice in 100 runs of the five refresh-lock suites against each other under load, both times as `timed out waiting for 1 fetch(es); saw 0`. Pointing `DARIO_CC_PATH` at a one-line file under identical load takes the same measurement to 58-133 ms, which accounts for all of it. The suite now awaits `detectCCOAuthConfig()` once before anything is on a clock — nothing it asserts is about the scan. Not a product race: the detector memoizes in-process and caches to disk, so a real dario pays this once.
