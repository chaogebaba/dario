...
| `--session-affinity` / `--no-session-affinity` / `DARIO_SESSION_AFFINITY` / config `sessionAffinity.enabled` | Wrap the selected pool strategy with session-sticky routing. Explicit client/session identifiers are preferred, then body identifiers and a stable message fallback. Bindings are model-scoped, fail over on unavailable accounts, and expire after the configured idle TTL. | on |
