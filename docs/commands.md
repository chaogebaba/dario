...
| `--pool-strategy=<headroom\|fill-first\|round-robin>` / `DARIO_POOL_STRATEGY` | Where new conversations land in a multi-account pool. `headroom` spreads them to the seat with the most slack; `round-robin` cycles eligible aliases with a cursor per model family; `fill-first` concentrates them on the alphabetically-first eligible seat until it drains to the 2% floor, then spills to the next. See [Multi-account pool](./multi-account-pool.md#routing-strategy). | `headroom` |
| `--session-affinity` / `--no-session-affinity` / `DARIO_SESSION_AFFINITY` / config `sessionAffinity.enabled` | Wrap the selected pool strategy with session-sticky routing. Native and explicit client session identifiers are preferred, followed by stable body conversation identifiers. Request IDs, prompt-cache keys, legacy user IDs, and first-message hashes are diagnostic-only because they are not reliably unique per session. Identity-less requests use the pool strategy without creating a binding. Bindings are model-scoped, fail over on unavailable accounts, and expire after the configured idle TTL. | on |
| `--session-affinity-ttl=<ms>` / `DARIO_SESSION_AFFINITY_TTL_MS` / config `sessionAffinity.ttlMs` | Idle TTL for session-to-account bindings. Activity refreshes the TTL; failed bindings are released immediately. | `3600000` |
...
| `--skip-egress-check` / `DARIO_SKIP_EGRESS_CHECK=1` | Start even when the egress-IP check fails. By default `dario proxy` refuses to start with an egress proxy configured but unusable — every local signal (URL parsed, bridge bound, SOCKS handshake completed) is also consistent with traffic still leaving by the default route, so the route is not proven until a remote endpoint reports back. Use this when you would rather have dario up and returning errors than not up at all. It never falls back to direct. (v5.6) | off |
| `DARIO_EGRESS_IP_URL` / config `egressIpUrl` | Endpoint asked "what address do you see?". Parsed tolerantly: Cloudflare's `key=value` trace, a bare address, or JSON carrying `ip` / `origin` / `address`. Anything that is not a valid IP is treated as a failed check rather than an answer. (v5.6) | `https://cloudflare.com/cdn-cgi/trace` |
