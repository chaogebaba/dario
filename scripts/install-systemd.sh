...
# Escape a value for interpolation into a unit file.
#
# systemd expands % specifiers in ExecStart, WorkingDirectory,
# ReadWritePaths and friends, and % is legal in a path on every
# filesystem dario runs on. A checkout under ~/pct%test would silently
# become .../pct/run/user/1000est — %t is the runtime directory — and
# `systemd-analyze verify` returns 0 without a word about it. %% is the
# escape. Nothing here rejects the path; it just stops meaning something
# else.
unit_escape() { printf '%s' "${1//%/%%}"; }
...
install_service() {
  ...
  #
  # Every interpolated path goes through unit_escape (% specifiers) and is
  # quoted (systemd splits ExecStart on whitespace, so a checkout under
  # "~/my projects" would otherwise arrive as three arguments).
  local e_bun e_entry e_repo e_home
  e_bun="$(unit_escape "$bun_bin")"
  e_entry="$(unit_escape "$entry")"
  e_repo="$(unit_escape "$REPO_ROOT")"
  e_home="$(unit_escape "$DARIO_HOME")"
